...
#include <thread>
#include <SDL.h>
#include "NESEmu/Cartridge/Loader/ArchaicINes.hpp"
#include "NESEmu/Cartridge/Loader/INes.hpp"
#include "NESEmu/Cartridge/Loader/INes2.hpp"
...
struct GraphicManager {
    ...
    uint32_t getEmphasizedColor(uint32_t color, bool emphasizeRed, bool emphasizeGreen, bool emphasizeBlue) const {
        // Get components
        uint8_t red = color >> 16;
        uint8_t green = (color >> 8) & 0xFF;
        uint8_t blue = color & 0xFF;

        // Check what is to be attenuated
        bool attenuateRed = emphasizeGreen | emphasizeBlue;
        bool attenuateGreen = emphasizeRed | emphasizeBlue;
        bool attenuateBlue = emphasizeRed | emphasizeGreen;

        // Attenuate if necessary // TODO: https://www.nesdev.org/wiki/NTSC_video
        if (attenuateRed) {
            red = (red * 0.746f) - 0.0912f;
        }

        if (attenuateGreen) {
            green = (green * 0.746f) - 0.0912f;
        }

        if (attenuateBlue) {
            blue = (blue * 0.746f) - 0.0912f;
        }
    ...
    void plotPixel(unsigned int x, unsigned int y, uint8_t color, bool emphasizeRed, bool emphasizeGreen, bool emphasizeBlue) {
        ...
        _pixelsBuffers[getPixelIndexInCurrentBuffer(x, y)] = getEmphasizedColor(paletteColor, emphasizeRed, emphasizeGreen, emphasizeBlue);
...
int main(int argc, char *argv[]) {
    ...
    cartridgeFactory.registerLoader(std::shared_ptr<NESEmu::Cartridge::Loader::Interface>(new NESEmu::Cartridge::Loader::ArchaicINes()));
    cartridgeFactory.registerLoader(std::shared_ptr<NESEmu::Cartridge::Loader::Interface>(new NESEmu::Cartridge::Loader::INes()));
    cartridgeFactory.registerLoader(std::shared_ptr<NESEmu::Cartridge::Loader::Interface>(new NESEmu::Cartridge::Loader::INes2()));
