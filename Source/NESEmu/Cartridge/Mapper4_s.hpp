...
void Chip<TCpuHardwareInterface, TPpuHardwareInterface>::cpuReadPerformed(TCpuHardwareInterface &cpuHardwareInterface) {
    ...
    if ((address >= 0x6000) && (address < 0x8000)) {
        ...
        if ((this->hasPrgRam()) && _prgRamChipEnable) {
            // Read Prg-Ram
    ...
    // Prg-Rom
        ...
        if (address < 0xA000) {
            bank = (_prgRomBankMode) ? ((this->getPrgRomSize() >> 13) - 2)  : _bankSelect[6];
        }
        else if (address < 0xC000) {
            bank = _bankSelect[7];
        }
        else if (address < 0xE000) {
            bank = (_prgRomBankMode) ? _bankSelect[6] : ((this->getPrgRomSize() >> 13) - 2);
        }
        else {
            bank = (this->getPrgRomSize() >> 13) - 1;
        }
        
        // Read Prg-Rom
        cpuHardwareInterface.setDataBus(this->readPrgRom((bank << 13) | (address & 0x1FFF)));
    }
}
...
void Chip<TCpuHardwareInterface, TPpuHardwareInterface>::cpuWritePerformed(TCpuHardwareInterface &cpuHardwareInterface) {
    ...
    if ((address >= 0x6000) && (address < 0x8000)) {
        ...
        if ((this->hasPrgRam()) && _prgRamChipEnable && !_prgRamWriteProtection) {
            // Write Prg-Ram
...
void Chip<TCpuHardwareInterface, TPpuHardwareInterface>::ppuReadPerformed(TPpuHardwareInterface &ppuHardwareInterface) {
    ...
    if (address < 0x2000) {
        if (this->hasChrRam()) {
            // Read Chr-Ram
            ppuHardwareInterface.setDataBus(this->readChrRam(address));//TODO: voir si chr-ram est comme chr-rom (banked)
        } else {
            uint16_t mask;
            uint8_t bank;
            ...
            // Read Chr-Rom
            ppuHardwareInterface.setDataBus(this->readChrRom((bank << 10) | (address & mask)));
        }
    }
