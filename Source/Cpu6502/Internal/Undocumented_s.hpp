...
template <class TConfiguration>
void Chip<TConfiguration>::arr0() {
    // Store ALU result in accumulator
    _accumulator = _alu.getAdderHold();

    // Carry is bit 6 and Overflow is bit 6 XOR bit 5
    _flagsHelper.clear<Flag::Carry, Flag::Overflow>();
    _flagsHelper.set<Flag::Carry>((_accumulator & 0x40) != 0x0);
    _flagsHelper.set<Flag::Overflow>(((_accumulator & 0x40) != 0x0) ^ ((_accumulator & 0x20) != 0x0));
...
template <class TConfiguration>
void Chip<TConfiguration>::arrImm0() {
    _currentInstruction = &Chip::arrImm1;
    immediate();
}

template <class TConfiguration>
void Chip<TConfiguration>::arrImm1() {
    // The AND is not performed by the ALU but by bus conflict which cause a low level to win (it is like an AND operation)
    // See : https://wiki.nesdev.com/w/index.php/Bus_conflict
    // And : https://wiki.nesdev.com/w/index.php/Programming_with_unofficial_opcodes
    ror(_accumulator & _inputDataLatch);

    // Fetch next opcode during performing ALU
    fetchOpcode(&Chip::arr0);
}

// OAL = ORA #$EE + AND + TAX

template <class TConfiguration>
void Chip<TConfiguration>::oal0() {
    // Store ALU result in accumulator and xIndex
    _accumulator = _alu.getAdderHold();
    _xIndex = _accumulator;
...
template <class TConfiguration>
void Chip<TConfiguration>::oalImm0() {
    _currentInstruction = &Chip::oalImm1;
    immediate();
}

template <class TConfiguration>
void Chip<TConfiguration>::oalImm1() {
    // Perform AND
    // The OR is not performed by the ALU but by bus conflict ??? TODO: a voir
    // See : https://wiki.nesdev.com/w/index.php/Bus_conflict
    // And : https://wiki.nesdev.com/w/index.php/Programming_with_unofficial_opcodes
    _alu.performAnd((_accumulator | 0xFF), _inputDataLatch);    // TODO: normalement la constante est 0xEE mais j'ai du mettre 0xFF pour passer la test rom inst_test-v5, maintenant en réalité la constante peut etre n'importe quoi, c'est un opcode non stable

    // Fetch next opcode during performing ALU
    fetchOpcode(&Chip::oal0);
}

// SAX

template <class TConfiguration>
void Chip<TConfiguration>::saxImm0() {
    _currentInstruction = &Chip::saxImm1;
    immediate();
}

template <class TConfiguration>
void Chip<TConfiguration>::saxImm1() {
    // Substracting data fetched to data to compare them
    // The AND is not performed by the ALU but by bus conflict which cause a low level to win (it is like an AND operation)
    // See : https://wiki.nesdev.com/w/index.php/Bus_conflict
    // And : https://wiki.nesdev.com/w/index.php/Programming_with_unofficial_opcodes
    _alu.performSum<DecimalSupported, true>(_accumulator & _xIndex, _inputDataLatch, false, true);

    // Fetch next opcode during performing ALU (don't save result, just affect flags)
    fetchOpcode(&Chip::finishAluForXIndex<Flag::Carry, Flag::Zero, Flag::Negative>);
}

template <class TConfiguration>
template <_Detail::FlagsHelper::Flag ...EFlags>
void Chip<TConfiguration>::finishAluForXIndex() {
    // Store ALU result in xIndex
    _xIndex = _alu.getAdderHold();
...
template <class TConfiguration>
void Chip<TConfiguration>::anc0() {
    // Perform AND
    _alu.performAnd(_accumulator, _inputDataLatch);

    // Fetch next opcode during performing ALU
    fetchOpcode(&Chip::anc1);
}

template <class TConfiguration>
void Chip<TConfiguration>::anc1() {
    // Store ALU result in accumulator
    _accumulator = _alu.getAdderHold();

    // Copy bit 7 to Carry flag
    _flagsHelper.clear<Flag::Carry>();
    _flagsHelper.set<Flag::Carry>((_accumulator & 0x80) != 0x0);
...
template <class TConfiguration>
void Chip<TConfiguration>::ancImm0() {
    _currentInstruction = &Chip::anc0;
    immediate();
}
